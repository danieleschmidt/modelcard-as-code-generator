...
class WandbIntegration:
    """Integration with Weights & Biases for automatic model card generation."""
    ...
    def from_run(self, run_path: str, config: Optional[CardConfig] = None) -> ModelCard:
        ...
        try:
            ...
            self._extract_artifacts(card, run)
            ...
            card.metadata.update({
                ...
                "wandb_finished_at": run.finished_at
            })
            
            logger.info(f"Generated model card from W&B run {run.name}")
            return card
    ...
    def from_project(self, project_path: str, filters: Optional[Dict[str, Any]] = None) -> List[ModelCard]:
        ...
        try:
            ...
            for run in runs:
                try:
                    card = self.from_run(f"{project_path}/{run.id}")
                    cards.append(card)
                except Exception as e:
